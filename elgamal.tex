...
Weiterhin ist anzumerken, dass das ElGamal"=Verfahren eng mit dem Diffie"=Hellman"=Schema verwandt ist. Dies ist kein Zufall, den Taher El Gamal erkannte 1985, dass das Diffie"=Hellman Schlüsselaustauschverfahren zu einem Public"=Key Verschlüsselungsverfahren erweitert werden kann. [... S. 400] % TODO Introduction to modern ...
...
Beide Teilnehmer müssen sich über eine zyklische Gruppe $\mathbb{G} = \langle g \rangle$ der Ordnung $p$ einigen, sowohl $\mathbb{G}$ als auch der Erzeuger $g$ werden öffentlich gemacht. [...] % TODO Zitat: Beutelsbacher Kryptoglogie ISBN 978-3-658-05976-7 Seite 147
Das National Institute of Standards and Technology (oder kurz NIST) hat eine Auswahl an empfohlenen Parametern veröffentlicht. [... S. 404] % TODO modern crypto

\paragraph{Erzeugung der Schlüssel} ~\\
Der Empfänger muss zunächst eine zufällige natürlich Zahl $b \in \{1,..,p-1\}$ als geheimen Schlüssel wählen. Anschließend erzeugt der Empfänger B seinen öffentlichen Schlüssel. Er berechnet hierfür das Gruppenelement $\beta = g^b \in \mathbb{G}$ und gibt dieses bekannt.

\paragraph{Verschlüsseln} ~\\
Möchte nun A eine Nachricht $m \in \mathbb{G}$ an B senden, muss A zunächst eine zufällige natürlich Zahl $a \in \{1,..,p-1\}$ wählen und daraus das Gruppenelement $\alpha = g^a \in \mathbb{G}$ berechnen. Anschließend bestimmt A den Schlüssel $k = \beta^a \in \mathbb{G}$. A wendet nun einen Verschlüsselungalgorithmus $f$ unter Verwendung des Schlüssel $k$ auf die Nachricht $m$ an. Den resultierenden Geheimtext $c = f_k(m) \in \mathbb{G}$ sowie das Element $\alpha$ sendet A an B.
Im ursprünglichen ElGamal"=Schema wurde $f$ definiert als: $f_k(x) = k*x \; mod \; p$ in einer primen Restklassengruppe der Ordnung $p$. % TODO Zitat: Beutelsbacher Kryptoglogie ISBN 978-3-658-05976-7 Seite 147
...
Damit kann nun der Geheimtext entschlüsselt werden, indem einfach die Umkehrfunktion $f_k^{-1}$ des Verschlüsselungalgorithmus angewendet wird.
...
Konkret wählen wir die Gruppe $\mathbb{G} = (\mathbb{Z} / 47\mathbb{Z})^\times$ und wie schon im Beispiel zum Babystep"=Giantstep"=Algorithmus stellt sich heraus, dass $g=5$ ein geeigneter Erzeuger unserer zyklischen Gruppe ist. Anschließend werden die Parameter, welche die Gruppe definieren, $p=47$ und $g=5$ veröffentlicht.
...
Notice the presence of a rather mysterious"=looking $1^n$ as an input parameter to many algorithms [...]. This is a common ``hack'' found in complexity"=theory literature. We want the running time of these algorithms to by polynomial"=time in the security parameter $n$ -- by passing $1^n$ as a parameter, we ensure that the input length is of length at least $n$; and hence the algorithm gets to run for time polynomial in $n$. [...] % TODO http://drona.csa.iisc.ernet.in/~arpita/Cryptography15/Scribe1.pdf
\end{quote}

Bevor wir uns wieder dem DDH"=Problem zuwenden, wollen wir noch ein kurzes Lemma einführen. Sei $\mathbb{G}$ eine endliche Gruppe und $m \in \mathbb{G}$ ein zufälliges Element. Das Lemma besagt nun, dass die Multiplikation von $m$ mit einem zufälligen Element $k$ der Gruppe, zu gleichmäßig verteilten Elementen $k'$ der Gruppe führt. Somit ist die Verteilung der $k'$ nicht von $m$ abhängig und damit kann aus $k'$ keine Information gewonnen werden, welche Rückschlüsse auf $m$ zulässt. [... S400] % TODO modern crypto seite 400

\paragraph{Lemma ...} Sei $\mathbb{G}$ eine endliche Gruppe und $m \in \mathbb{G}$ willkürlich gewählt. Wird $k \gets \mathbb{G}$ zufällig gewählt und ist $k' := k*m$, dann erhält man die selbe Verteilung für $k'$, als wenn $k' \gets \mathbb{G}$ gewählt wird.
Anders gesagt, gilt für alle $\hat{g} \in \mathbb{G}$:
$$
\mathrm{Pr}[k*m=\hat{g}]=1/|\mathbb{G}|,
$$
wobei die die Wahrscheinlichkeit durch eine zufälligen Wahl von $k \in \mathbb{G}$ bestimmt wird.

\paragraph{Beweis ...} Sei $\hat{g} \in \mathbb{G}$ beliebig. Dann gilt:
$$
...
\begin{itemize}
 ...
 \item $\mathsf{Enc}$: Der Verschlüsselungalgorithmus nimmt den öffentlichen Schlüssel $pk$ mit $pk = \langle \mathbb{G},q,g,x \rangle$ und eine Nachricht $m \in \mathbb{G}$ entgegen. Es wird ein zufälliges Element $y \in (\mathbb{Z} / q\mathbb{Z})^\times$
...
\begin{itemize}
 ...
 \item Es wird ein zufälliges Bit $b \in \{0, 1\}$ gewählt, woraufhin der Geheimtext $c \gets \mathsf{Enc}_{pk}(m_b)$ bestimmt wird und an $\mathcal{A}$ übergeben wird.
...
\paragraph{Definition ...} Das Verschlüsselungsverfahren $\Pi = (\mathsf{Gen, Enc, Dec})$ besitzt nicht zu unterscheidende Geheimtexte aus Sicht eines Abhörers, wenn für alle $\mathsf{PPT}$ Angriffsalgorithmen $\mathcal{A}$ eine vernachlässigbare Funktion $\mathsf{negl}$ existiert, für die gilt:
$$
...
\paragraph{Satz ...} \hspace{1em} Wenn das DDH"=Problem schwer in Bezug auf $\mathcal{G}$ ist, dann generiert das ElGamal"=Verschlüsselungsverfahren nicht zu unterscheidende Geheimtexte unter Angriffen mit ausgewählten Klartexten. Anders gesagt das Verfahren garantiert die Ununterscheidbarkeit von Geheimtexten.

\paragraph{Beweis ...} Sei $\Pi$ das ElGamal"=Verschlüsselungsverfahren. Wir zeigen nachfolgend, dass $\Pi$ nicht zu unterscheidende Geheimtexte unter Anwesenheit eines Beobachters generiert.
...
Aus Lemma ... % TODO
folgt, dass die zweite Komponente des Geheimtextes des Verfahrens $\tilde{\Pi}$ ein gleichmäßig zufällig verteiltes Element der Gruppe $\mathbb{G}$ ist. Weiterhin besteht keine Abhängigkeit zur Nachricht $m$ welche zu verschlüsseln ist, da aus Lemma ... % TODO
folgt, dass $g^z$ ein zufälliges Element von $\mathbb{G}$ ist wenn $z$ aus $\mathbb{Z_q}$ zufällig gewählt wurde.
...
Nun wollen wir folgenden $\mathsf{PPT}$ Algorithmus $D$ betrachten, welcher das gegebene DDH"=Problem relativ zu $\mathcal{G}$ zu lösen versucht. Der Algorithmus $D$ erhält das Tupel $(\mathbb{G},q,g,h_1,h_2,h_3)$, es gilt: $h_1=g^x$, $h_2=g^y$ und $h_3$ ist entweder $g^{xy}$ oder $g^z$, wenn $x$, $y$ und $z$ zufällig gewählt wurden. Der Algorithmus $D$ sollte nun herausfinden, welcher dieser beiden Fälle zutrifft.
...
Da $D$ nur eine 1 zurück gibt, wenn die Ausgabe $b'$ von $\mathcal{A}$ gleich zu $b$ ist, somit gilt:
$$
...
Das Verfahren hat allerdings auch Nachteile, wie beispielsweise der Nachrichtenraum, welcher durch die gewählt Gruppe $\mathbb{G}$ bestimmt wird. Für manche Gruppen ist es jedoch möglich eine umkehrbare Kodierung von Gruppenelementen zu Bitstrings zu definieren und dieses Problem damit zu umgehen. [... S. 404] % TODO modern crypto

Weiterhin ist das Verfahren sehr rechenintensiv, da häufig starker Pseudozufallszahlen generiert werden müssen. Es ist daher auch nicht besonders zur Verschlüsselung großer Datenmengen geeignet. Hier bietet es sich an das ElGamal-Verfahren nur zur Verschlüsselung eines Schlüssels zu verwenden, welcher wiederum für ein symmetrisches Verfahren genutzt wird. Ein solches Konstrukt wird als hybrides Verschlüsselungverfahren bezeichnet. [...] % TODO https://www.cs.utexas.edu/~rashid/395Tcrypt/2_1.pdf
